...
namespace ClientB
{
    class Program
    {
        ...
        private static string roleClaimVlaue = "sub";
        ...
        {
            Console.WriteLine("Press any key to start client B");
            Console.ReadKey();
            ...
            Task task = coapClient.ObserveAsync(resourceUriString1, new Action<CodeType, string, byte[]>(ObserveAction));
            Task.WhenAll(task);
            ...
            bool sending = true;
            int index = 0;
            //publish messages via CoAP to a resource 
            while (sending)
            {
                Task sendTask = coapClient.PublishAsync(resourceUriString2,
                        "text/plain",
                        Encoding.UTF8.GetBytes(String.Format("hello from CoAP {0}", index++)),
                        false,
